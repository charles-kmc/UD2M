            ...
            for max_unfolded_iter in MAX_UNFOLDED_ITER:
                ...
                if args.use_wandb:
                    ...
                    wandb.init(
                            ...
                            name = f"{formatted_time}_num_steps_{num_timesteps}_unfolded_iter_{args.max_unfolded_iter}_{init}_task_{args.task}_rank_{args.rank}_eta{args.eta}_zeta_{args.zeta}_epoch_{args.epoch}_T_AUG_{T_AUG}",
                        )
                # data
                ...
                # directories
                root_dir_results = os.path.join(args.path_save, args.task, "Results", args.eval_date, f"Max_iter_{max_iter}", f"unroll_iter_{args.max_unfolded_iter}", f"timesteps_{num_timesteps}", f"Epoch_{args.epoch}", f"T_AUG_{T_AUG}", f"zeta_{zeta}", f"eta_{eta}")
