...
def get_dataloader(dataset: VisionDataset,args, subset_data="train"):
    ...
    sampler = RandomSampler(dataset, 
                            ...
                            replacement=False
                        ) if args.data.is_random_sampler else None
...
class ImageNetDataset(VisionDataset):
    def __init__(self, root_dataset: str, im_size:int, subset_data:str = "train"):
        ...
        fpaths = glob(os.path.join(root_dataset,"ImageNet", f"{subset_data}", f"{subset_data}" + '/*.JPEG')) + glob(os.path.join(root_dataset,"ImageNet", f"{subset_data}", f"{subset_data}" + '/*.png'))
        assert len(fpaths) > 0, f"File list is empty. Check the root_dataset. {os.path.join(root_dataset,"ImageNet", f"{subset_data}", f"{subset_data}" + '/*.JPEG')}"
