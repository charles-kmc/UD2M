...
for num_timesteps in 3 ; do
    ...
    if [ "$general" = "True" ]; then
        ...
        if [ "$task" = "inp" ]; then
            ...
            ckpt_dir="/users/cmk2000/sharedscratch/CKM/Regularised_CDM/ckpts_RAM_unfolded_3_rank_5_test/imagenet/sigma_model_0.01/deblur/motion_5/16-06-2025/kernel_size_5"
            ckpt_name="imagenet_BestPSNR_lora_deblur_motion_epoch=690-5.ckpt"
    ...
    if [ $fid_eval -eq 0 ]; then
        ...
        # python3 fid.py \
        #         -d "/users/cmk2000/sharedscratch/CKM/Regularised_CDM/Results_RAM_unfolded_iter_3_rank_5_paper/LSUN/sr_lora/operator_gaussian_3/var_0.01/22-06-2025/ddim/factor_4/timesteps_$num_timesteps/zeta_0.9/eta_0.8/lambda_0.3" \
