...
#             2> "$RESULTS_DIR"_err_train_dif_$i.error
    ...
    ckpt_dir=/users/js3006/conddiff/UD2M/Results/ablation_${i}/ckpts_ud2m_inp
    ckpt_name="mnist_lora_inp_random_epoch=399-5.ckpt"
    op="random"            # gaussian, uniform, motion
    ...
    for num_timesteps in {1,}; do
        ...
        echo "Computing FID: lambda: $lamb and sigma $sigma on dataset_name $dataset_name"          
        python3 fid.py \
                -d Results/ablation_$i/MNIST/inp_operator_random/ddim/K_${i}_N_${num_timesteps}_samples_16_eq \
